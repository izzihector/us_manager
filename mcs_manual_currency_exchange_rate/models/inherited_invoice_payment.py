...
class AccountPayments(models.Model):
    ...
    # TPW Note: add by tpw, 2022.02.15
    ...
    def default_get(self, fields):
        ...
        move_id = self.env['account.move'].search([('id', 'in', self._context.get('active_ids'))])

        # TPW Note: Replace 2022.02.15
        # result.update({
        ...
        # })
        if move_id:
            move_id = move_id.filtered(lambda move: move.is_invoice(include_receipts=True))
            if move_id:
                result.update({
                    'apply_manual_currency_exchange':move_id.apply_manual_currency_exchange,
                    'manual_currency_exchange_rate':move_id.manual_currency_exchange_rate,
                    'inverted_currency_exchange_rate': move_id.inverted_currency_exchange_rate,
                    })
