...
class ProductBrand(models.Model):
    ...
    def write(self,value):
        ...
        if value.get('brand_id'):
            ...
            for record in self:
                brand_brw.write({'product_ids': [(4, record.id)]})
                products = self.env['product.product'].search([('product_tmpl_id', '=', record.id)])
                ...
                for product_variants in products:
                    product_variants.update({'brand_id': record.brand_id.id})
        return brand
